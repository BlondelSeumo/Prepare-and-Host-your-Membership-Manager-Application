<?php
   ...
   * @copyright 2020
   * @version $Id: content.class.php, v1.00 2020-04-20 18:20:24 gewa Exp $
   */
  if (!defined("_WOJO"))
  ...
  class Content
  {
	  ...
	  public function processPage()
	  {
		  ...
		  if (empty(Message::$msgs)) {
			  $data = array(
				  ...
				  'page_type' => empty($_POST['membership_id']) ? "normal" : "membership",
			  ...
			  $message = Filter::$id ? 
			  Message::formatSuccessMessage($data['title'], Lang::$word->PG_UPDATE_OK) : 
			  Message::formatSuccessMessage($data['title'], Lang::$word->PG_ADDED_OK);
	  ...
	  public function processCoupon()
	  {
          ...
          if($_POST['type'] == "p") {
			  $rules['discount'] = array('required|numeric|min_numeric,1|max_numeric,99', Lang::$word->DC_DISC);
		  }
		  $validate = Validator::instance();
		  $safe = $validate->doValidate($_POST, $rules);
	  ...
	  public static function rendertCustomFieldsFront($id = '')
	  {
		  ...
		  if ($id) {
			  ...
			  AND cf.active = ? 
			  ORDER BY cf.sorting;";
			  $data = Db::run()->pdoQuery($sql, array($id, 1))->results();
		  } else {
			  $data = Db::run()->select(self::cfTable, null, array("active" => 1), "ORDER BY sorting")->results();
		  }
      ...
      public function renderNews()
      {
		  ...
		  return Db::run()->select(self::nTable, null, array("active" => 1), "ORDER BY created DESC")->results();
	  ...
	  public function renameFile()
	  {
		  ...
		  if (empty(Message::$msgs)) {
			  ...
			  $row = Db::run()->first(self::fTable, null, array("id" => Filter::$id));
