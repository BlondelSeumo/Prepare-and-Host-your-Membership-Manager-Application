<?php
   ...
   * @copyright 2020
   * @version $Id: users.class.php, v1.00 2020-04-05 10:12:05 gewa Exp $
   */
  ...
  class Users
  {
      ...
      public function Index()
      {
          ...
          if (isset($_GET['letter']) and $find) {
              ...
              $counter = self::$db->count(false, false, "SELECT COUNT(*) FROM `" . self::mTable . "` $where AND `fname` LIKE '%" . trim($find) . "%' OR `lname` LIKE '%" . trim($find) . "%' AND `fname` REGEXP '^" . $letter . "'");
              $and = "AND `fname` LIKE '%" . trim($find) . "%' AND `lname` LIKE '%" . trim($find) . "%' AND `fname` REGEXP '^" . $letter . "'";

          } elseif (isset($_POST['find'])) {
              $counter = self::$db->count(false, false, "SELECT COUNT(*) FROM `" . self::mTable . "` $where AND `fname` LIKE '%" . trim($find) . "%' OR `lname` LIKE '%" . trim($find) . "%'");
	  ...
	  public function Edit($id)
	  {
		  ...
		  if (!$row = Db::run()->first(self::mTable, null, array("id =" => $id, "AND type <>" => "owner"))) {
			  ...
			  $tpl->userfiles = Db::Run()->select(Content::fTable)->results();
			  $tpl->template = 'admin/users.tpl.php';
		  }
	  }
      ...
      public function _addUser($rules, $filters)
      {
          ...
          if (empty(Message::$msgs)) {
              ...
              $data = array(
				  ...
				  'notes' => $safe->notes,
				  'user_files' => empty($_POST['user_files']) ? 0 : Utility::implodeFields($_POST['user_files']),
